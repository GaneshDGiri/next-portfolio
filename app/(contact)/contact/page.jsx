  ...
  return (
    <section className="relative flex items-center justify-center py-6" id="contact">
      ...
      <div className="container mx-auto px-4 sm:px-6 py-12 sm:py-16 md:py-20 flex flex-col items-center">
        ...
        >
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-primary">Get In Touch</h2>
          <p className="text-muted-foreground max-w-xl mx-auto text-sm sm:text-base px-4">
            Have a project in mind? Looking to collaborate? Drop me a message, and let's create
            ...
            <div className="space-y-4 sm:space-y-6">
              {contactInfo.map((info, index) => (
                <div key={index} className="flex items-start space-x-3 sm:space-x-4">
                  <div className="p-2 rounded-lg bg-secondary/10 flex-shrink-0">{info.icon}</div>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {info.href ? (
                      ...
                      <p className="text-sm sm:text-base text-primary break-words">{info.value}</p>
                    )}
                  </div>
                </div>
              ))}
